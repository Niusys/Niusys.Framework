...
namespace Niusys.Extensions.Buses
{
    public interface IMongoStore<TCollection>
    {
        ...
        Task<IList<TCollection>> SearchAsync(FilterDefinition<TCollection> lockedFilter, SortDefinition<TCollection> sort, int limit, int skip = 0, CancellationToken cancellationToken = default);
        ...
        Task<bool> DeleteAsync(ObjectId messageId, CancellationToken cancellationToken = default);
        Task<TCollection> GetByIdAsync(ObjectId messageId, CancellationToken cancellationToken = default);
    }
}
